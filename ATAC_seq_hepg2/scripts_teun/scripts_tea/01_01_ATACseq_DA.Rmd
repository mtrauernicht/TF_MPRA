...
wrap_ma_plots <- function(result_list) {
  ...
  df <- transform(
    df,
    cat  = make_cat(df),
    treatment = factor(treatment, levels = drugs)
  )
...
exp <- DESeqDataSet(exp, ~ replicate + celltype + treatment)
```
...
```{r}
exp$celltype <- droplevels(exp$celltype)
...
ggplot(df, aes(rip, sizefactor)) +
  ...
  geom_point(aes(fill = celltype, shape = treatment),
             size = 3) +
...
pcplot <- ggplot(df, aes(pc1, pc2)) +
  ...
  geom_line(linetype = replicate, values = c("solid", "longdash", "dotted"), name = "Replicate")+
  scale_fill_celltype(guide = fill_shape_guide()) +
  scale_shape_manual(values = c(25, 21:24),
                     name = "Treatment") +
  labs(
...
drugs <- setNames(nm = tail(levels(exp$treatment), -1))

treatment <- lapply(exps, function(tmp) {
  lapply(drugs, function(i) {
...
```{r}
k562 <- wrap_ma_plots(treatment$k562)
k562$plot
```
