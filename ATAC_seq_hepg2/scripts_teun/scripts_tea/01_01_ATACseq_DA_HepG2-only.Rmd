...
scale_colour_replicate <- function(...) {
  scale_colour_manual(
    name = "replicate",
    values = c(
      "R1"  = "#e41a1c",
...
wrap_ma_plots <- function(result_list) {
  ...
  df <- data.table::rbindlist(df, idcol = "treatment")
  df <- transform(
    df,
    cat  = make_cat(df),
  ...
  g <- ggplot(df, aes(baseMean, log2FoldChange)) +
    ...
    scale_y_continuous(
      ...
      name = expression("Log"[2]*" Fold Change vs DMSO")
    ) +
    scale_colour_manual(
      name = "Direction",
...
parallel_sets_plot <- function(data) {
  data$id <- unname(unlist(lapply(split(data$treatment, data$treatment), seq_along)))
  ids <- with(data, sort(unique(id[padj < 0.05 & !is.na(padj)])))
  data <- subset(data, id %in% ids)
  
  ggplot(data, aes(treatment, id = id, split = cat, value = 1)) +
  geom_parallel_sets(
...
exp <- DESeqDataSet(exp, ~replicate + treatment)
```
...
ggplot(df, aes(rip, sizefactor)) +
  ...
  geom_point(aes(fill = replicate, shape = treatment),
             size = 3) +
  scale_fill_replicate(guide = fill_shape_guide()) +
...
```{r}
exp <- estimateDispersions(exp, quiet = TRUE, fitType = "local")

df <- as.data.frame(rowData(exp))
df <- df[order(df$baseMean),]

ggplot(df, aes(baseMean, dispMAP)) +
  ggtitle("Mean-dispersion trend") + theme(plot.title = element_text(hjust = 0.5)) + theme (plot.title = element_text(color="black", size=16, face="bold")) +
...
pcplot <- ggplot(df, aes(pc1, pc2)) +
  ...
  geom_point(aes(fill = factor, shape = treatment), size = 3) +
  scale_fill_replicate(guide = fill_shape_guide()) +
  scale_shape_manual(values = c(25, 21:24),
                     name = "Treatment") +
  labs(
...
#  tmp <- exp
# tmp$celltype <- relevel(tmp$celltype, tag)
#  nbinomWaldTest(tmp)
# })
# 
# treatments <- setNames(nm = tail(levels(exp$treatment), -1))
# 
# treatment <- lapply(exps, function(tmp) {
#   lapply(treatments, function(i) {
#     results(tmp, contrast = c("treatment", i, "DMSO"))
#   })
# })
...
diff_acc <- results(exp1, contrast = c("treatment", "DMSO", "CDCA"))
...
ggmaplot(diff_acc, main = expression("DMSO" %->% "CDCA"),
...
### Rifampicin
```{r}
diff_acc <- results(exp1, contrast = c("treatment", "DMSO", "Rifampicin"))
diff_acc$name <- 1:nrow(diff_acc)

ggmaplot(diff_acc, main = expression("DMSO" %->% "Rifampicin"),
   fdr = 0.05, fc = 0.5, size = 1,
   palette = c("#B31B21", "#1465AC", "darkgray"),
   genenames = as.vector(diff_acc$name),
   legend = "top", top = 100,
...
##T0901317
```{r}
diff_acc <- results(exp1, contrast = c("treatment", "DMSO", "T0901317"))
diff_acc$name <- 1:nrow(diff_acc)

ggmaplot(diff_acc, main = expression("DMSO" %->% "T0901317"),
   fdr = 0.05, fc = 0.5, size = 1,
   palette = c("#B31B21", "#1465AC", "darkgray"),
   genenames = as.vector(diff_acc$name),
   legend = "top", top = 100,
...
### Wy-14643
```{r}
diff_acc <- results(exp1, contrast = c("treatment", "DMSO", "Wy-14643"))
diff_acc$name <- 1:nrow(diff_acc)

ggmaplot(diff_acc, main = expression("DMSO" %->% "Wy-14643"),
   fdr = 0.05, fc = 0.5, size = 1,
   palette = c("#B31B21", "#1465AC", "darkgray"),
   genenames = as.vector(diff_acc$name),
   legend = "top", top = 100,
...
bigWigs = c("/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq/bigwig/HepG2_DMSO_R1.bw","/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq/bigwig/HepG2_DMSO_R2.bw","/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq/bigwig/HepG2_T0901317_R1.bw", "/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq/bigwig/HepG2_T0901317_R2.bw")

#Extract the signal for the locus of interest
track_data = track_extract(bigWigs = bigWigs, loci = "chr19:44,752,434-45,085,747")

#Summarize tracks by condition
track_data = track_summarize(summary_list = track_data, condition = c("DMSO R1", "DMSO R2", "T0901317 R1","T0901317 R2"), stat = "mean")

#Basicplot
track_plot(summary_list = track_data, draw_gene_track = TRUE, build = "hg38", query_ucsc = FALSE)

#Highlight regions of interest and add RNA track
markregions = data.frame(chr = c("chr19", "chr19", "chr19"), start=c("44914325","44942238"," 44929988"), end = c("44919346","44949565", "44941562"), name = c("APOC1","APOC4-APOC2", "enhancer APOC1"))
track_plot(summary_list = track_data, draw_gene_track = TRUE, gene_model = "/DATA/usr/t.filipovska/projects/ATAC-seq_TF-footprinting/analysis/references/hg_ucsc.gtf.gz", isGTF = TRUE, 
           col = c("darkgreen","darkgreen","mediumorchid4", "mediumorchid4"),
           show_ideogram = TRUE, build = "hg38", regions= markregions, boxcol = "gray70", query_ucsc=FALSE)

#Pring as PDF
# pdf(file = "/DATA/usr/t.filipovska/projects/ATAC-seq_TF-footprinting/analysis/Plots/hepG2_DMSO_T09_APOC1_APOC4APOC2_IGV.pdf")
# track_plot(summary_list = track_data, draw_gene_track = TRUE, gene_model = "/DATA/usr/t.filipovska/projects/ATAC-seq_TF-footprinting/analysis/references/hg_ucsc.gtf.gz", isGTF = TRUE, 
#            col = c("darkgreen","darkgreen","mediumorchid4", "mediumorchid4"),
#            show_ideogram = TRUE, build = "hg38", regions= markregions, boxcol = "gray70", query_ucsc=FALSE)
# dev.off()
...
DMSO_R1 <- import.bw("/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq/bigwig/HepG2_DMSO_R1.bw", as="GRanges")

target <- DataTrack(DMSO_R1, chromosome ="chr19")
```
