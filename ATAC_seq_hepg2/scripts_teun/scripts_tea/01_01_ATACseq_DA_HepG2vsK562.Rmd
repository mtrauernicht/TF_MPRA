...
```{r}
#filter here 
exp <- DESeqDataSet(exp, ~ replicate + celltype)
```
...
dispersion <- ggplot(df, aes(baseMean, dispMAP)) +
...
pcplot <- ggplot(df, aes(pc1, pc2)) +
  ...
  geom_point(aes(fill = celltype, shape = replicate), size = 3) +
  scale_fill_celltype(guide = fill_shape_guide()) +
  scale_shape_manual(values = c(25, 21:24),
                     name = "Replicate") +
  labs(
...
```
```{r}
exp_2 <- data.frame(assay(exp))
plot(exp_2$HepG2_DMSO_R1, exp_2$HepG2_DMSO_R2, xlim=c(0, 1200), ylim=c(0,1000))
plot(exp_2$K562_DMSO_R1, exp_2$K562_DMSO_R2)
```
...
diff_acc <- results(exp1, contrast = c("celltype", "K562", "HepG2"))
diff_acc$name <- 1:nrow(diff_acc)

ggmaplot(diff_acc, main = expression("K562" %->% "HepG2"),
...
ggmaplot(diff_acc, main = expression("HepG2" %->% "K562"),
   fdr = 0.05, fc = 0.5, size = 1,
   palette = c("#B31B21", "#1465AC", "darkgray"),
