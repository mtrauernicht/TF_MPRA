...
bam_dir <- "/DATA/usr/t.filipovska/projects/ATAC-seq_TF-footprinting/analysis/bam/6420"
...
files <- read.table("atac_fastq_table.tsv", 
...
cmd_bwa <- function(read1, read2, output) {
              path <- "/DATA/usr/t.filipovska/software/Miniconda3/bin"
...
for (i in seq_len(nrow(bam_table))) {
  ...
  tmpfiles1 <- gsub("_R1.+", "_temp.bam", r1)
