...
bw_dir <- "/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq/bigwig/"
...
for (group in names(files)) {
  ...
  factor <- size_factor$rip[size_factor$sample == group] / 1e6
  
  data <- coverage(data) / factor
  data <- data[names(data) %in% seqnames]
  export.bw(data, paste0("/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq/bigwig/", group, ".bw"))
}
