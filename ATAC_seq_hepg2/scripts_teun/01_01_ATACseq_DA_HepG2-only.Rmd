...
exp <- readRDS("/DATA/usr/t.filipovska/projects/ATAC-seq_TF-footprinting/analysis/rds/experiment_6420.rds")
```
...
row_keep <- rowSums(assay(exp)) > thres_count
...
exp_rpm_matrix <- exp_counts_long %>%
  dplyr::select(-reads_in_peaks) %>%
  dcast(id ~ condition, value.var = 'rpm') %>%
  column_to_rownames(var = "id")

metadata <- data.frame("condition" = names(exp_rpm_matrix)) %>%
  mutate(replicate = gsub(".*(R[1-3])", "\\1", condition),
         condition = gsub("_R[1-3]", "", condition)) %>%
  unique()

# exp <- DESeqDataSet(exp, ~treatment)

exp_deseq <- DESeqDataSetFromMatrix(countData = round(exp_rpm_matrix),
                                     colData = metadata,
                                     design= ~ condition)
```
...
```{r}
# Differential analysis
exp_deseq <- DESeq(exp_deseq)
diff_results <- results(exp_deseq,
                       contrast = c("condition", "HepG2_T0901317", "HepG2_DMSO"))
diff_results <- cbind(diff_results, data.frame(exp@rowRanges[,1]))



# MA plots with significant hits
ggmaplot(diff_results, main = expression("DMSO" %->% "T0901317"),
   fdr = 0.2, fc = 0.5, size = 1,
...
hepg2 <- wrap_ma_plots(treatment$hepg2)
hepg2$plot
```
