...
for (i in unique(tib_fimo_2$motif_id)) {
  write.table(tib_fimo_2[tib_fimo_2$motif_id == i,] %>% dplyr::select(-motif_id),
              file = paste("/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq_hepg2/motifs/all/", i, ".bed", sep = ""),
              row.names = F,
              col.names = F,
              quote = F,
              sep = '\t')
...
difftf_activity <- read_tsv("/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq_hepg2/diffTF/output_rna_new/FINAL_OUTPUT/extension100/HepG2vsK562.top1000.summary.tsv") %>%
...
reporter_activity <- read_csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/ATAC_seq_hepg2/diffTF/tf_reporter_activity.csv")
...
reporter_activity <- reporter_activity %>%
  ...
  dplyr::select('TF' = tf, HepG2_DMSO, HepG2_CDCA, K562_DMSO)
reporter_activity_dif <- reporter_activity %>%
  mutate(reporter_activity = log2(K562_DMSO / HepG2_DMSO)) %>%
  dplyr::select(TF, reporter_activity)

activity_comp <- merge(difftf_activity, reporter_activity_dif)
...
activity_comp <- merge(activity_comp, rnaseq_matrix, by.x = 'TF', by.y = 'hgnc_symbol')
...
activity_comp <- merge(activity_comp, tf_class)
