...
for (i in unique(bc_df_2$condition)) {
  ...
  cor$cor[cor$condition == i] <- cor(x, y$pDNA, "pairwise.complete.obs")
}
...
# bc_exclude <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/pDNA_insert_seq/results/bc_exclude.csv") %>% select(-X) %>% setnames("x", "barcode")
# exclude <- bc_df_cDNA_filt[!bc_df_cDNA_filt$barcode %in% bc_exclude$barcode,]
# 
# # Reassign barcodes with wrong inserts attached
# bc_replace <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/pDNA_insert_seq/results/bc_replace.csv") %>% select(-X, -bc.match)
