---
title: "Barcode processing - pMT02 - stimulation 2"
...
bc_files = list.files('/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6139_stimulation-2/results/',
...
bc_annotation <- bc_annotation %>% dplyr::select(TF, Spacing, Distance, Barcode, Promoter, Background,
                                          barcode, seq)
...
bc_df <- bc_df %>% dplyr::select(-TF, -Spacing, -Distance, -Barcode,
                                                                -Promoter, -Background, -seq)
```


### Add barcode annotation to barcode counts & extract first bc read count information
```{r annotation, out.width= "80%", fig.align= "center", echo=FALSE, warning= FALSE}
...
```{r out.width= "80%", fig.align= "center", echo=FALSE, warning= FALSE, message=FALSE}
## Attach pDNA data
pDNA <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf5927_stimulation-1/results/mt20200504_bc_df_levenshtein.csv", header = T, stringsAsFactors = F) %>%
  dplyr::select(barcode, pDNA_1, pDNA_2)
bc_df <- merge(pDNA, bc_df, all = T)
...
pDNA <- data.frame("pDNA" = bc_df$rpm[bc_df$condition == "pDNA_1"],
                   "barcode"= bc_df$barcode[bc_df$condition == "pDNA_1"], stringsAsFactors=FALSE)
...
```{r normalization, out.width= "80%", fig.align= "center", echo=FALSE, warning= FALSE}
...
# bc_exclude <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/pDNA_insert_seq/results/bc_exclude.csv") %>% dplyr::select(-X) %>% setnames("x", "barcode")
# exclude <- bc_df_cDNA_filt[!bc_df_cDNA_filt$barcode %in% bc_exclude$barcode,]
# 
# # Reassign barcodes with wrong inserts attached
# bc_replace <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/pDNA_insert_seq/results/bc_replace.csv") %>% dplyr::select(-X, -bc.match)
...
bc_df_rep <- bc_df_cDNA_filt[bc_df_cDNA_filt$hPGK == "No" & bc_df_cDNA_filt$native_enhancer == "No" &
  ...
  dplyr::select(Barcode, activity, condition, reporter_id)
rep1 <- bc_df_rep[bc_df_rep$Barcode == 1,] %>% dplyr::select(-Barcode)
rep2 <- bc_df_rep[bc_df_rep$Barcode == 2,] %>% dplyr::select(-Barcode)
rep3 <- bc_df_rep[bc_df_rep$Barcode == 3,] %>% dplyr::select(-Barcode)
rep4 <- bc_df_rep[bc_df_rep$Barcode == 4,] %>% dplyr::select(-Barcode)
rep5 <- bc_df_rep[bc_df_rep$Barcode == 5,] %>% dplyr::select(-Barcode)
rep6 <- bc_df_rep[bc_df_rep$Barcode == 6,] %>% dplyr::select(-Barcode)
rep7 <- bc_df_rep[bc_df_rep$Barcode == 7,] %>% dplyr::select(-Barcode)
rep8 <- bc_df_rep[bc_df_rep$Barcode == 8,] %>% dplyr::select(-Barcode)
...
setwd("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6139_stimulation-2/results/")
