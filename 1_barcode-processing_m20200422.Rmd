...
bc_files = list.files('/DATA/usr/m.trauernicht/projects/tf_activity_reporter/data/SuRE_TF_1/results/',
...
x <- read_tsv("/DATA/usr/m.trauernicht/projects/tf_activity_reporter/data/SuRE_TF_1/results/vitA_r1_TGACCAAT_S100_barcodes.tsv")
y <- read_tsv("/DATA/usr/m.trauernicht/projects/tf_activity_reporter/data/SuRE_TF_1/results/2i_neg_LIF_r1_CGATGTAT_S98_barcode_counts.tsv")
```
...
experimental_reads <- data.frame(colSums(bc_df_new_match[,c(-1, -24:-32)])) %>% 
...
bc_nomatch <- bc_df_nomatch$barcode[bc_df_nomatch$bc_sum > 100]
...
for (i in unique(cor_bc_reads$match_barcode)) {
  for (j in unique(cor_bc_reads$rep)) {
    ...
    n <- n + length(i)
    percent <- round(((n / length(unique(cor_bc_reads$match_barcode)))*100),1)
    progress <- paste("progress:", percent/3, "%")
    if (percent %% 10 == 0) {
    print(progress)
    }
  }
...
for (i in unique(bc_df$barcode)) {
  if (i %in% cor_bc_reads$non_match_barcode) {
    ...
    n <- n + length(i)
    percent <- round(((n / length(unique(bc_df$barcode)))*100),1)
    progress <- paste("progress:", percent, "%")
    if (percent %% 10 == 0) {
    print(progress)
    }
...
filename <- SetFileName("_barcodes_processed", "mt")
