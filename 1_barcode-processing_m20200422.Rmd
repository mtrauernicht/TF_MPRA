...
pDNA_0 <- nrow(pDNA[pDNA$pDNA < 20,])
pDNA_1 <- nrow(pDNA[pDNA$pDNA >= 20,])
...
lbls <- c("#pDNA barcode reads >= 20", "#pDNA barcode reads <20")
lbls <- paste(lbls, ": ", round(percentage*100,2), "%", sep = "")
pie(slices, labels = lbls, col = c("#6F7D95","#B2BFBF"))


## How many cDNA reads do I loose through pDNA filtering?
cDNA <- bc_df[bc_df$pDNA < 20,]
...
pDNA_0 <- pDNA[pDNA$bc_counts < 20,]

ggplot(data = pDNA, aes(x = length)) + 
  geom_density(fill ="#AAACB0", alpha = 0.5) + geom_density(data = pDNA_0, fill ="#BFEDC1", alpha = 0.5) +
  xlab("TF reporter length") + labs(title = "Length distribution of TF reporters", subtitle = "green: >20 pDNA counts, grey: >= 20 pDNA counts") + 
  theme_classic()


# Remove all barcodes that did not appear in pDNA data
bc_df[is.na(bc_df)] <- 0
bc_df <- bc_df[bc_df$pDNA >= 20,]
...
pDNA <- bc_df[grep("pDNA", bc_df$condition),] %>% 
  select(barcode, bc_counts, activity) %>% unique() %>%
  setnames(c("bc_counts", "activity"), c("pDNA_counts", "pDNA_activity")) %>%
  mutate(pDNA_counts = ave(pDNA_counts, barcode, FUN = function(x) mean(x)),
         pDNA_activity = ave(pDNA_activity, barcode, FUN = function(x) mean(x))) %>%
  unique()
bc_df_cDNA <- merge(pDNA, bc_df_cDNA, all = T)
...
sp.plot <- arrangeGrob(sp1,sp2,sp3, nrow = 3)
