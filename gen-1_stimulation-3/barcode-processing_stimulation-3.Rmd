...
bc_annotation <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/library_design/output/mt20191218_tf-array.csv", header = T) %>% 
  select(barcode, TF, Spacing, Distance, Barcode, Promoter, Background)
```
...
bc_df_pDNA <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf5927_stimulation-1/results/mt20200504_bc_df_levenshtein.csv", header = T) %>% 
  select(pDNA_1, pDNA_2, barcode) %>%
  mutate(starcode_counts = (pDNA_1 + pDNA_2) / 2) %>%
  select(barcode, starcode_counts) %>%
  mutate(condition = "pDNA")
bc_df <- rbind(bc_df, bc_df_pDNA)
...
for (i in unique(bc_df_2$condition)) {
  x <- bc_df_2[bc_df_2$condition == i,] %>% select(rpm, barcode)
  y <- bc_df_2[bc_df_2$condition == "pDNA",] %>% select(rpm, barcode) %>% setnames("rpm", "pDNA")
  z <- merge(x,y, all = T)
  cor$cor[cor$condition == i] <- cor(z$rpm, z$pDNA, "pairwise.complete.obs")
}
...
bc_df <- bc_df[bc_df$n_bc >= 1600,] %>% select(-n_bc)
...
## Compute activity by dividing through pDNA data
 ...
 for (i in unique(bc_df$barcode)) {
    ...
    pDNA <- bc_df$rpm[bc_df$barcode == i & bc_df$condition == "pDNA"]
