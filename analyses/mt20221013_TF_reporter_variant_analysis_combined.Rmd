...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf7027_KD_mES/results/mt20221013_reporter_activity_filt_combined.csv", header = T) 

# Remove non-existent data: barcodes that 
cDNA_df_remove <- cDNA_df %>%
  filter(library == "1" & nchar == 13)

cDNA_df <- anti_join(cDNA_df, cDNA_df_remove) %>%
  dplyr::select(-tf_activity, -reporter_id2, 'pval_minP' = pval, -mutated_activity, -nfya_activity, -id, -pDNA, -starcode_counts, -nchar, -transfection, -rpm, -reporter_rpm,
                ...
                -sample_id, -sample, -replicate, -pval_adj_minP)  %>%
  distinct()
...
cDNA_df2 <- cDNA_df %>%
  ...
  dplyr::filter(neg_ctrls == "No", 
                ...
                native_enhancer == "No") %>%
  distinct(tf, condition, reporter_activity_minP, pval_minP) %>%
  mutate(reporter_activity_minP = log2(ave(reporter_activity_minP, tf, condition, FUN = mean))) %>%
  mutate(pval_minP = -log10(ave(pval_minP, tf, condition, FUN = mean))) %>%
  distinct()

# Compare similar samples
cDNA_df4 <- cDNA_df2 %>%
  dplyr::select(-pval_minP) %>%
...
#cDNA_df2$reporter_activity_minP[cDNA_df2$pval_minP < -log10(0.15) | cDNA_df2$reporter_activity_minP < 1.5 & cDNA_df2$reporter_activity_minP > -0.5] <- 0 # Select only reporters that are significantly active

cDNA_df2$reporter_activity_minP[cDNA_df2$pval_minP < -log10(0.35) | cDNA_df2$reporter_activity_minP < 1.25 & cDNA_df2$reporter_activity_minP > -0.15] <- 0 # Select only reporters that are significantly active

cDNA_df3 <- cDNA_df2 %>%
  filter(condition %in% c("A549", "HCT116", "HEK293", "HepG2", "K562", "MCF7", "mES_2i_LIF", "NPC", "U2OS")) %>%
  dplyr::select(-pval_minP) %>%
  mutate(sum = ave(reporter_activity_minP, tf, FUN = function(x) sum(x, na.rm = T))) %>%
...
cDNA_df3 <- cDNA_df2 %>% 
  ...
  mutate(sd_tf_activity = ave(reporter_activity_minP, tf, FUN = sd)) %>%
  mutate(max_tf_activity = ave(reporter_activity_minP, tf, FUN = max)) %>%
  mutate(min_tf_activity = ave(reporter_activity_minP, tf, FUN = min)) %>%
