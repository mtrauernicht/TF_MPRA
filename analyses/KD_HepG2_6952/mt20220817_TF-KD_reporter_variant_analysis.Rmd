...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6952_KD_HepG2/results/mt20220825_reporter_activity_filt_combined.csv", header = T) %>%
  dplyr::select(-tf_activity, -reporter_id2, 'pval_minP' = pval, -pval_adj, -mutated_activity)

# The siRNA of TP53 & JUN were empty, so this can be essentially counted as NT
cDNA_df$sample[cDNA_df$condition == "TP53"] <- "NT_r3"
cDNA_df$sample_id[cDNA_df$condition == "TP53"] <- "NT_r3_gcf6952"
cDNA_df$condition[cDNA_df$condition == "TP53"] <- "NT"
cDNA_df$sample[cDNA_df$condition == "JUN"] <- "NT_r4"
cDNA_df$sample_id[cDNA_df$condition == "JUN"] <- "NT_r4_gcf6952"
cDNA_df$condition[cDNA_df$condition == "JUN"] <- "NT"

# Rename KD conditions so they match the TF names
nm1 <- c("sample", "condition", "sample_id", "id") 
cDNA_df[nm1] <- lapply(cDNA_df[nm1], gsub, pattern = "TCF1", replacement = "HNF1A")
cDNA_df[nm1] <- lapply(cDNA_df[nm1], gsub, pattern = "FOS", replacement = "FOS::JUN")
cDNA_df[nm1] <- lapply(cDNA_df[nm1], gsub, pattern = "STAT1", replacement = "STAT1::STAT2")
cDNA_df[nm1] <- lapply(cDNA_df[nm1], gsub, pattern = "^NFE2_", replacement = "MAF::NFE2_")
cDNA_df[nm1] <- lapply(cDNA_df[nm1], gsub, pattern = "^NFE2$", replacement = "MAF::NFE2")
cDNA_df[nm1] <- lapply(cDNA_df[nm1], gsub, pattern = "TFCP2L2", replacement = "GRHL1")
...
cDNA_df2 <- merge(cDNA_df, nt_df, all = T) %>%
  ...
  distinct(tf, reporter_id, condition, reporter_FC) %>%
