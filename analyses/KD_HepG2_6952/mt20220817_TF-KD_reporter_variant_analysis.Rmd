...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6952_KD_HepG2/results/mt20220901_reporter_activity_filt_combined.csv", header = T) %>%
...
for (i in unique(tf_kd_targets2$condition)) {
  ...
  p <- ggplot(tf_kd_targets2 %>%
    ...
    geom_quasirandom(aes(alpha = 0.7), dodge.width = .75) +
    scale_color_manual(values = c(colors_diverse[1], colors_diverse[3])) +
    ggtitle(i) +
    theme_classic_lines_90()
...
cDNA_df_TF <- merge(cDNA_df, nt_df, all = T) %>%
  ...
  mutate(reporter_FC = log2(reporter_activity_minP/nt_reporter_activity_minP)) %>%
  mutate(tf_condition = paste(tf, condition, sep = "_")) %>%
  distinct(reporter_FC, tf, condition, tf_condition, reporter_id, promoter, spacing, distance, background) %>%
...
ggplot_custom(cDNA_df_TF3 %>% 
       ...
       aes(x = -log10(pval_adj), color = is_target)) +
  geom_density() +
...
exp_variance2 <- exp_variance %>%
  filter(pval_adj <= 0.01, is_target == T) %>%
