...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6952_KD_HepG2/results/mt20220817_reporter_activity_filt_combined.csv", header = T) %>%
...
## Same but now only identical matches to check direct KDs
cDNA_df5 <- merge(cDNA_df4, nt_df, all = T) %>%
  na.omit() %>%
  mutate(reporter_FC = log2(reporter_activity_minP/nt_activity)) %>%
...
tf_kd_targets <- read.csv2("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6952_KD_HepG2/tf_kd_targets.csv") %>% 
  ...
  setnames("target", "tf") %>%
  mutate(is_target = T) %>%
  full_join(cDNA_df5, by = c("condition", "tf")) 
...
pca_res <- prcomp(cDNA_df5, scale. = TRUE)
loadings <- data.frame('scale' = pca_res$scale) %>%
  dplyr::filter(scale > 0.5)
g <- ggbiplot(pca_res, labels = rownames(cDNA_df5))
```
...
for (i in unique(cDNA_df2$condition)) {
  print(i)
  p <- ggplot(cDNA_df2 %>% filter(condition == i), mapping = aes(x = reorder(tf, -mean_tf_activity), y = reporter_FC)) +
    geom_boxplot() +
    geom_quasirandom() +
    ggtitle(i) +
    theme_classic_lines_90()
