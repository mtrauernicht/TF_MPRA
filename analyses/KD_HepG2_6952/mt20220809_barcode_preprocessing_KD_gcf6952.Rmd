...
pMT09_02 <- bc_df_filt %>%
  filter(str_detect(sample_id, "E1961_pDNA_r1_gcf6992"), library == "1+2") %>%
  dplyr::select(barcode, sample_id, rpm) %>%
  spread(sample_id, rpm) %>%
  #mutate(pMT09_02 = (`pMT02-09_v2_r1_gcf6952` + `pMT02-09_v3_r1_gcf6952`)/2) %>%
  dplyr::select(barcode, "pMT09_02" = `E1961_pDNA_r1_gcf6992`)
...
pMT09_02 <- bc_df %>%
  filter(sample_id == "E1961_pDNA_r1_gcf6992") %>%
   filter(starcode_counts > 10) %>%
   dplyr::select(barcode, "E1961_pDNA" = rpm)
...
bc_df$activity <- bc_df$rpm / bc_df$E1961_pDNA
...
bc_df_cDNA_filt <- bc_df_cDNA_filt %>%
  dplyr::select(-match, -E1961_pDNA, -mean_activity, -deviation, -n_reporters)
