...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6502_gen2/results/mt20210708_reporter_activity_filt.csv", header = T)
...
cDNA_df <- cDNA_df[cDNA_df$pDNA_counts_rpm >6,]

##remove K562 stressed and R1 T09
cDNA_df <- cDNA_df %>% filter(cDNA_df$sample!= "K562_stressed", cDNA_df$sample!="R1_HepG2_T090")
...
## re-calculate reporter activity with only pDNA<=10
cDNA_df$replicate_activity <- ave(cDNA_df$activity, cDNA_df$reporter_id, 
                                cDNA_df$sample, FUN =
                                  function(x) mean(x))

cDNA_df$reporter_activity <- ave(cDNA_df$activity, cDNA_df$reporter_id, cDNA_df$condition, FUN = function(x) mean(x))
cDNA_df$reporter_activity_sd <- ave(cDNA_df$activity, cDNA_df$reporter_id, cDNA_df$condition, FUN = function(x) sd(x))
...
##Heatmap commerical reporters

selection <- c("GATA4", "HNF1A", "HNF4A", "NR1D1", "NR1H4::RXRA", "NR1I2", "NR1I3", "ONECUT1", "PPARA::RXRA", "RORA", "SOX9", "TCF7", "FOXA1", "GATA1", "REL", "WT1", "EGR1", "POU2F1", "SRF", "MTF1", "CREB1", "SP1", "NFYA", "STAT3", "XBP1", "IRF3","MAF::NFE2", "THRB")
cDNA_df$group[grep(paste(selection, collapse = "|"), cDNA_df$tf)] <- "selection"


cDNA_df <- cDNA_df[!is.na(cDNA_df$tf),]
cDNA_df <- cDNA_df[!is.na(cDNA_df$condition),]
...
# pheatmap function
pheatmap(as.matrix(tf_activity_heatmap[tf_activity_heatmap$group == "selection",][,-8]),
...
p <- pheatmap(as.matrix(t(tf_activity_heatmap[tf_activity_heatmap$group == "commercialVSassay",-8])),
         color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(100),
         breaks = myBreaks1, border_color = "black", 
         cellwidth = 12, cellheight = 10, cluster_rows = F,
         angle_col = 90)
...
#####check negative control activity
cDNA_df <- cDNA_df[!is.na(cDNA_df$tf),]
cDNA_df <- cDNA_df[!is.na(cDNA_df$condition),]
tf_activity_heatmap <- cDNA_df[cDNA_df$neg_ctrls == "Yes" & cDNA_df$hPGK == "No" &
                               cDNA_df$native_enhancer == "No" & cDNA_df$rand_promoter == "No" & cDNA_df$library == "2" & cDNA_df$commercial_reporter == "No",] %>% 
  dplyr::select(tf, condition, tf_activity) %>% unique()
tf_activity_heatmap <- dcast(tf_activity_heatmap, tf ~ condition, value.var="tf_activity")
group <- cDNA_df %>% dplyr::select(tf, group) %>% unique()
tf_activity_heatmap <- merge(tf_activity_heatmap, group)
...
ggplot(data = plot1 %>% dplyr::select(condition, tf_activity, tf, neg_ctrls) %>% unique(), aes(y = tf_activity,x = condition, alpha = 0.4)) + 
  geom_quasirandom(alpha = 0.4) + xlab("log2(cDNA/pDNA)") + 
  labs(title = "reporter activity distribution per condition") + scale_color_brewer(palette = "Set1") + theme_bw() + 
