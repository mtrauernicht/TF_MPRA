...
author: "Max Trauernich & Tea Filipovskat"
...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6502_gen2/results/mt20210722_reporter_activity_filt.csv", header = T)
...
cDNA_df$tf <- revalue(cDNA_df$tf, c("Tcf7l2" = "Tcf3", "Tcf7l2_neg" = "Tcf3_neg"))
cDNA_df$reporter_id <- gsub("Tcf7l2", "Tcf3", cDNA_df$reporter_id)
cDNA_df$tf <- revalue(cDNA_df$tf, c("TCF7L2" = "TCF3", "TCF7L2_neg" = "TCF3_neg"))
cDNA_df$reporter_id <- gsub("TCF7L2", "TCF3", cDNA_df$reporter_id)

# Remove potentially noisy data and recalculate activities
cDNA_df <- cDNA_df[cDNA_df$pDNA_counts_rpm >7,]
cDNA_df <- cDNA_df %>%
  mutate(replicate_activity = ave(activity, reporter_id, sample, FUN = function(x) mean(x)),
         reporter_activity = ave(activity, reporter_id, condition, FUN = function(x) mean(x)),
         log_reporter_activity = log2(reporter_activity)) %>%
  dplyr::select(-replicate_activity_sd, -reporter_activity_sd)


## label reporters which are in both library 1 and 2
cDNA_df$also_lib1 <- "No"
x <- unique(cDNA_df$tf[cDNA_df$library == "1"])
x <- toupper(x)
x <- gsub("_NEG", "_neg", x)
cDNA_df$also_lib1[cDNA_df$tf %in% x] <- "Yes"
...
ggplot(data = cDNA_df %>% dplyr::select(sample, log_reporter_activity, reporter_id, neg_ctrls) %>% unique(), aes(y = log_reporter_activity, x = "x")) + 
  ...
  labs(title = "reporter activity distribution per condition") + theme_bw() + facet_wrap(vars(sample))

## reporter activity distribution - highlight negative motif controls
ggplot(cDNA_df[cDNA_df$hPGK == "No" & cDNA_df$library == "2",], aes(x = log_activity, fill = neg_ctrls, alpha = 0.4)) + 
...
ggplot(cDNA_df %>% 
         filter(promoter != "hPGK", commercial_reporter == "No", library == "2", also_lib1 == "No") %>%
         mutate(tf = gsub("_neg", "", tf)) %>%
         dplyr::select(reporter_id, log_reporter_activity, neg_ctrls, promoter, tf) %>% 
         unique(), aes(y = log_reporter_activity, x = neg_ctrls, color = promoter, alpha = 0.4)) + 
  geom_quasirandom(dodge.width = 0.75) + xlab("mutated motif?") + 
...
ggplot(data = cDNA_df[cDNA_df$distance != "",] %>% 
         filter(neg_ctrls == "No", hPGK == "No", rand_promoter == "No", library =="2", also_lib1 == "No", commercial_reporter =="No") %>%
...
ggplot(data = cDNA_df[cDNA_df$spacing != "",] %>% 
         filter(neg_ctrls == "No", hPGK == "No", rand_promoter == "No", library =="1") %>%
...
ggplot(data = cDNA_df[cDNA_df$spacing != "",] %>% 
         filter(neg_ctrls == "No", hPGK == "No", rand_promoter == "No", library =="2", commercial_reporter == "No", also_lib1 == "No") %>%
         dplyr::select(log_reporter_activity, reporter_id, spacing, tf, condition) %>% 
         unique(), 
       aes(y = log_reporter_activity, x = condition, color = spacing, alpha = 0.4)) + 
  geom_quasirandom(dodge.width = 0.75) + ylab("log2(cDNA/pDNA)") + 
  labs(title = "Library 2: reporter activity distribution - spacings") +
  theme_bw() + facet_wrap(~tf) +
  scale_color_manual(values = colors)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 1)) 
...
ggplot(data = cDNA_df[cDNA_df$spacing != "",] %>% 
       ...
       aes(y = log_reporter_activity, x = condition, color = background, alpha = 0.4)) + 
  geom_quasirandom(dodge.width = 0.75) + ylab("log2(cDNA/pDNA)") + 
  labs(title = "Library 2: reporter activity distribution - backgrounds") +
  theme_bw() + facet_wrap(~tf) +
  scale_color_manual(values = colors)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 1)) 
...
ggplot(data = cDNA_df[cDNA_df$background != "" & cDNA_df$tf == "Stat3",] %>% 
  ...
  geom_boxplot(position = position_dodge(0.75)) +
  ylab("log2(cDNA/pDNA)") + 
  labs(title = "NR1H4::RXRA: reporter activity distribution - backgrounds") +
  scale_color_manual(values = colors)+
  theme_bw() +
  ylim(-1,5)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 1)) 
...
sp <- ggscatter(cDNA_df[cDNA_df$native_enhancer == "No" &
               ...
               cDNA_df$hPGK == "No" &
               cDNA_df$rand_promoter == "No" & cDNA_df$library=="1",],
...
sp2 <- ggscatter(cDNA_df[cDNA_df$native_enhancer == "No" &
               cDNA_df$neg_ctrls  == "No" &
               cDNA_df$hPGK == "No" &
               cDNA_df$rand_promoter == "No" & cDNA_df$library=="2",],
               x = "pDNA_counts_rpm", y = "rpm",
               size = 0.1, alpha = 0.3, color = "#2D3047", 
   add = "reg.line",
   add.params = list(color = "blue", fill = "lightgray"), title = "normalized barcode counts cDNA vs. pDNA",
   conf.int = TRUE, ylab = "cDNA", xlab = "pDNA")
sp2 + stat_cor(method = "pearson", label.x = 150, label.y = 2) + 
  geom_abline(linetype = "dashed") + facet_wrap(~tf) + xlim(0,200) + ylim(0,200)
...
cDNA_df$tf_activity <- ave(cDNA_df$replicate_activity, cDNA_df$sample, cDNA_df$tf, FUN = function(x) mean(x))
...
cDNA_df <- cDNA_df[!is.na(cDNA_df$sample),]
tf_activity_heatmap <- cDNA_df[cDNA_df$neg_ctrls == "No" & cDNA_df$hPGK == "No" &
                               cDNA_df$native_enhancer == "No" & cDNA_df$rand_promoter == "No" & cDNA_df$library == "2" & cDNA_df$commercial_reporter == "Yes",] %>% 
  dplyr::select(tf, sample, tf_activity) %>% unique()
tf_activity_heatmap <- dcast(tf_activity_heatmap, tf ~ sample, value.var="tf_activity")
...
# pheatmap function
pheatmap(as.matrix(t(tf_activity_heatmap[,-18])),
...
tf_activity_heatmap <- tf_activity_heatmap[,c(3,10,15,4,6,12,17,7,13,2,9,5,11,16,8,14,1)]
for (i in unique(tf_activity_heatmap$group)) {
  p <- pheatmap(as.matrix(t(tf_activity_heatmap[tf_activity_heatmap$group == i,-18])),
...
# nuclear_receptor <- c("AR", "ESR1", "HNF4A", "NR1D1", "NR1H2", "NR1H4::RXRA", "NR1I2", "NR1I3", "NR3C2", "NR4A1", "NR4A2::RXRA", "NR5A2", "PGR", "PPARA::RXRA", "PPARG::RXRA", "RAR:RXR", "RORA", "RXRA", "THRA", "THRB", "VDR::RXRA")
# cDNA_df$group[grep(paste(nuclear_receptor, collapse = "|"), cDNA_df$tf)] <- "Nuclear receptors"
# # 
# hepatic <- c("GATA4", "HNF1A", "HNF4A", "NR1D1", "NR1H4::RXRA", "NR1I2", "NR1I3", "ONECUT1", "PPARA::RXRA", "RORA", "SOX9", "TCF7")
# cDNA_df$group[grep(paste(hepatic, collapse = "|"), cDNA_df$tf)] <- "hepatic"
# 
# erythroid <- c("GATA1", "REL", "WT1")
# cDNA_df$group[grep(paste(erythroid, collapse = "|"), cDNA_df$tf)] <- "erythroid"

tf_activity_heatmap <- cDNA_df[cDNA_df$neg_ctrls == "No" & cDNA_df$hPGK == "No" &
                               cDNA_df$native_enhancer == "No" & cDNA_df$rand_promoter == "No" & cDNA_df$library == "2" & cDNA_df$commercial_reporter == "No" & cDNA_df$tf != "RANDOM1" & cDNA_df$tf != "RANDOM2" & cDNA_df$tf != "RANDOM3" & cDNA_df$tf !="IRX3",] %>% 
  dplyr::select(tf, sample, tf_activity) %>% unique()
tf_activity_heatmap <- dcast(tf_activity_heatmap, tf ~ sample, value.var="tf_activity")
group <- cDNA_df %>% dplyr::select(tf, group) %>% unique()
tf_activity_heatmap <- merge(tf_activity_heatmap, group)
...
for (i in unique(tf_activity_heatmap$group)) {
  p <- pheatmap(as.matrix(t(tf_activity_heatmap[tf_activity_heatmap$group == i,-18])),
         color = colorRampPalette(brewer.pal(n = 8, name = "RhBu"))(100),
         breaks = myBreaks1, border_color = "black", 
         cellwidth = 12, cellheight = 10, cluster_rows = F,
         angle_col = 90)
  print(p) 
...
## nuclear receptor drug targets commercial reporters vs Max

commercialVSassay <- c("NR1H4::RXRA", "NR1I2", "NR1H2", "PPARA::RXRA", "PPARA::RXRA_romanov", "NR1H4::RXRA_TF-seq", "NR1H2_romanov", "NR1H2_TF-seq" )
cDNA_df$group[grep(paste(commercialVSassay, collapse = "|"), cDNA_df$tf)] <- "commercialVSassay"

cDNA_df <- cDNA_df[!is.na(cDNA_df$tf),]
cDNA_df <- cDNA_df[!is.na(cDNA_df$sample),]
tf_activity_heatmap <- cDNA_df[cDNA_df$neg_ctrls == "No" & cDNA_df$hPGK == "No" &
                               cDNA_df$native_enhancer == "No" & cDNA_df$rand_promoter == "No" & cDNA_df$library == "2",] %>% 
  dplyr::select(tf, sample, tf_activity) %>% unique()
tf_activity_heatmap <- dcast(tf_activity_heatmap, tf ~ sample, value.var="tf_activity")
group <- cDNA_df %>% dplyr::select(tf, group) %>% unique()
tf_activity_heatmap <- merge(tf_activity_heatmap, group)
...
p <- pheatmap(as.matrix(t(tf_activity_heatmap[tf_activity_heatmap$group == "commercialVSassay",-18])),
         color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(100),
         breaks = myBreaks1, border_color = "black", 
         cellwidth = 12, cellheight = 10, cluster_rows = F,
         angle_col = 90)
...
### Plotting only the most active reporters per TF in a heatmap per condition
```{r,  out.width= "80%", fig.align= "center", echo=FALSE, warning= FALSE, message = FALSE}
cDNA_df_filt <- cDNA_df[cDNA_df$neg_ctrls == "No" & cDNA_df$hPGK == "No" &
                          cDNA_df$native_enhancer == "No" & cDNA_df$rand_promoter == "No" & 
                          cDNA_df$library == "2" & cDNA_df$commercial_reporter == "No" & 
                          cDNA_df$tf != "RANDOM1" & cDNA_df$tf != "RANDOM2" & 
                          cDNA_df$tf != "RANDOM3" & cDNA_df$tf !="IRX3",] %>% 
  dplyr::select(tf, sample, condition, replicate_activity, reporter_id, tf_activity) %>% unique()


cDNA_df_filt_top <- cDNA_df_filt %>%
  dplyr::filter(sample != "K562_stressed",
                sample != "R1_Hepg2_T090") %>%
  mutate(reporter_activity = ave(replicate_activity, condition, reporter_id, FUN = function(x) mean(x))) %>%
  dplyr::select(tf, reporter_id, reporter_activity, condition) %>%
  unique()

# Select highest expressing reporters (average over all conditions)
cDNA_df_filt_top_sel <- cDNA_df_filt_top %>%
  dplyr::select(tf, reporter_id, reporter_activity) %>%
  mutate(reporter_activity = ave(reporter_activity, reporter_id, FUN = function(x) mean(x))) %>%
  dplyr::arrange(desc(reporter_activity)) %>%
  dplyr::group_by(tf) %>%
  dplyr::slice_head()

# Keep only TFs that have at least activity > X in one condition
cDNA_df_filt_tf_sel <- cDNA_df_filt_top[cDNA_df_filt_top$reporter_id %in% cDNA_df_filt_top_sel$reporter_id,] %>%
  group_by(tf, condition) %>%
  filter(reporter_activity > 6)
  
cDNA_df_filt_top_sel <- cDNA_df_filt_top_sel[cDNA_df_filt_top_sel$reporter_id %in% cDNA_df_filt_tf_sel$reporter_id,] %>%
  dplyr::select(tf, reporter_id) %>%
  filter(tf != "GATA4", tf != "NR1I2", tf != "TRP53", tf != "PAX6", tf != "SOX2", tf != "FOSL1", tf != "ETS2", tf != "ESR1")

# For GATA4 there seems to be a more specific reporter
cDNA_df_filt_top_gata4 <- cDNA_df_filt_top %>%
  filter(tf == "GATA4") %>%
  spread(condition, reporter_activity) %>%
  mutate(dif = `HepG2_DMSO` - `K562_DMSO`) %>%
  dplyr::arrange(desc(dif)) %>%
  dplyr::slice(2) %>%
  dplyr::select(tf, reporter_id)

cDNA_df_filt_top_sel <- rbind(cDNA_df_filt_top_sel, cDNA_df_filt_top_gata4)

# For NR1I2 there seems to be a more specific reporter
cDNA_df_filt_top_nr1i2 <- cDNA_df_filt_top %>%
  filter(tf == "NR1I2") %>%
  spread(condition, reporter_activity) %>%
  mutate(dif = `HepG2_Rifampicin` - `HepG2_DMSO`) %>%
  dplyr::arrange(desc(dif)) %>%
  dplyr::slice(1) %>%
  dplyr::select(tf, reporter_id)

cDNA_df_filt_top_sel <- rbind(cDNA_df_filt_top_sel, cDNA_df_filt_top_nr1i2)

# For TRP53 there seems to be a more specific reporter
cDNA_df_filt_top_trp53 <- cDNA_df_filt_top %>%
  filter(tf == "TRP53") %>%
  spread(condition, reporter_activity) %>%
  mutate(dif = `HepG2_DMSO` - `K562_DMSO`) %>%
  dplyr::arrange(desc(dif)) %>%
...
cDNA_df_filt_top_sel <- rbind(cDNA_df_filt_top_sel, cDNA_df_filt_top_trp53)


# Make matrix of these top reporters and combine with mean of other reporters
cDNA_df_filt_heatmap <- cDNA_df_filt_top[cDNA_df_filt_top$reporter_id %in% cDNA_df_filt_top_sel$reporter_id,] %>%
  mutate(reporter_activity = log2(reporter_activity)) %>%
  spread(condition, reporter_activity) %>%
  dplyr::select(-reporter_id)

## Take the mean of these TFs (because they seem to have noisy reporters as highest expressing ones)
tf_activity_heatmap <- cDNA_df_filt %>% 
  dplyr::select(tf, condition, tf_activity) %>% 
  mutate(tf_activity = ave(tf_activity, tf, condition, FUN = function(x) mean(x))) %>%
  unique()

## Plot all TF activities in one plot
tf_activity_heatmap$tf[tf_activity_heatmap$tf == "TRP53"] <- "P53"
data <- tf_activity_heatmap %>% filter(condition == "HepG2_DMSO_FBS")
ggplot(data, 
       aes(x = 'tf', y = tf_activity, color = tf_activity)) +
  geom_quasirandom() +
  geom_label(data = data %>% filter(tf_activity > 50), 
             aes(label = tf),
             nudge_x = 0.1) +
  theme_pubr(border = T) +
  scale_color_viridis_b(option = "inferno", end = 0.8) +
  xlab("") +
  ylab("TF reporter activity - HepG2")

## Transform for heatmap plotting
tf_activity_heatmap <- dcast(tf_activity_heatmap, tf ~ condition, value.var="tf_activity")
tf_activity_heatmap <- tf_activity_heatmap[!tf_activity_heatmap$tf %in% cDNA_df_filt_heatmap$tf,]

tf_activity_heatmap <- rbind(tf_activity_heatmap, cDNA_df_filt_heatmap) %>%
  filter(tf != "PAX6")

# Last selection steps
cDNA_df_filt_heatmap <- cDNA_df_filt_heatmap %>% 
  remove_rownames %>% column_to_rownames(var="tf")

myBreaks1 <- seq(0, 6, 0.06)

cDNA_df_filt_heatmap <- cDNA_df_filt_heatmap[,c("K562_DMSO", "HepG2_DMSO")]

pheatmap(as.matrix(t(cDNA_df_filt_heatmap)),
...
pheatmap(as.matrix(t(tf_activity_heatmap[tf_activity_heatmap$tf == "HNF1A", -8])),
         color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(100),
         breaks = myBreaks1, border_color = "#565656", 
         cellwidth = 10, cellheight = 10,
         annotation_col = annotation_col, 
...
HepG2_enhanced <- c("GATA4", "HNF1A", "HNF4A", "NR1D1", "NR1H4::RXRA", "NR1I2", "ONECUT1", "PPARA::RXRA", "RORA", "SOX9", "TCF7")
tf_activity_df$group[grep(paste(HepG2_enhanced, collapse = "|"), tf_activity_df$tf)] <- "HepG2_enhanced"

K562_enhanced <- c("GATA1", "REL", "WT1", "EGR1", "POU2F1", "GRHL1", "SRF", "MTF1", "CREB1", "KLF4", "SP1", "NFYA", "STAT3", "XBP1", "IRF3","MAF::NFE2")
...
tf_activity_df <- tf_activity_df[-grep("neg|promega|TF-seq|romanov|RANDOM", tf_activity_df$reporter),] %>%
  dplyr::select(tf, reporter, group, "HepG2_DMSO")

tf_activity_df <- tf_activity_df %>%
  mutate(mean = ave(HepG2_DMSO, tf, FUN = function(x) mean(x)),
         sd = ave(HepG2_DMSO, tf , FUN = function(x) sd(x)))


ggplot(data = tf_activity_df[tf_activity_df$group != "Other",] , aes(x = tf, y = `HepG2_DMSO`, color = `HepG2_DMSO`)) +
  geom_quasirandom() +
  theme_bw() +
...
cDNA_df_TF <- cDNA_df[cDNA_df$native_enhancer == "No" &
  ...
  dplyr::select(log_reporter_activity, tf, spacing, distance, background) %>%
  unique()
...
### Calculate expression variance
exp_variance <- data.frame(c("spacing", "distance", "background", "Residuals"))
prediction <- data.frame(1:504)
names(prediction) <- "id"
names(exp_variance) <- "feature"
weight <- data.frame(c("(Intercept)", "spacing5bp", "distance21bp", "background2", "background3"))
names(weight) <- "feature"
...
for (i in unique(cDNA_df_TF$tf[cDNA_df_TF$tf!="RFX1" & cDNA_df_TF$tf!="THRB"])) {
  x <- lm(log_reporter_activity ~ spacing + distance + background, 
...
# Further plotting preparations
level_order <- c("2", "3", "10bp", "21bp", "5bp")
...
#for (i in unique(cDNA_df_TF$tf)) {
  ...
  # Plot to show prediction accuracy
  sp <- ggscatter(cDNA_df_TF[cDNA_df_TF$tf == "NR1H4::RXRA",], x = "log_reporter_activity", y = "activity_predicted",
   add = "reg.line", 
   add.params = list(color = "blue", fill = "lightgray"),
   conf.int = TRUE, ylab = "Predicted log2-activity", 
   title = "NR1H4::RXRA",
  ...
  # Feature importance overview for each TF (including mean activities per feature)
  fp <- ggplot(exp_variance[exp_variance$tf == "NR1H4::RXRA",], 
       aes(x = reorder(feature, -value), y = value, fill = group)) +
  scale_fill_manual(values = c("#1B998B", "#2D3047")) +
  geom_bar(stat = "identity") + 
  ylab("Variance explained (%)") + xlab("") +
  ...
  # Plot weights of parameters in model - this helps to understand the model 
  p <- ggplot(weight[weight$tf == "NR1H4::RXRA",], 
