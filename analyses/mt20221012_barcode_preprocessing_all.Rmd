...
metadata_df <- read_csv2("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/mt20231218_metadata_all.csv") %>%
...
bc_df_pert_dif <- bc_df_pert_dif_long %>%
  dplyr::select(reporter_id, "sample_id" = unique_sample, "reporter_activity_dif_sample" = activity_dif, tf_target) %>%
  distinct()
...
remove_pert_conditions <- c("A549_5aDHT", "A549_TCPOBOP", "HCT116_cDIM12", "HCT116_GSK4112", "HCT116_SR8278", "HCT116_SRI011381",
                            ...
                            "mES_RARA", "mES_RA", "mES_N2B27_RA", "mES_RXRA", "mES_Shh", "mES_N2B27_Shh", "mES_Sox2", "mES_Sox9", "mES_SRI011381",
                            "mES_vitA", "NPC_5aDHT", "NPC_ITE", "NPC_SR1078", "NPC_TCPOBOP", "U2OS_AICAR", "U2OS_Aldosteone", "U2OS_Cytosporone",
                            "U2OS_FK", "U2OS_LPS", "mES_4OHT", "mES_Dox")
...
bc_df_cDNA_filt$reporter_activity <- ave(bc_df_cDNA_filt$mean_activity_sample, bc_df_cDNA_filt$reporter_id, bc_df_cDNA_filt$condition, FUN = function(x, na.rm = T) mean(x))
...
bc_df_cDNA_filt_short <- bc_df_cDNA_filt %>%
  dplyr::select(-tf_activity, -reporter_id2, -mutated_activity, -pDNA, -starcode_counts, -nchar, -transfection, -rpm, -reporter_rpm,
