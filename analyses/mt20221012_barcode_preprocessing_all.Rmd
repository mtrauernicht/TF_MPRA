...
## Review which conditions to include - compute correlations between replicates of the same samples - only unstimulated
...
bc_df_sample <- bc_df_cDNA_filt %>%
  ...
  distinct(mean_activity_sample_minP, reporter_id, condition, unique_sample) %>% 
  mutate(mean_activity_sample_minP = log2(mean_activity_sample_minP)) %>%
  spread(unique_sample, mean_activity_sample_minP)

for (i in unique(bc_df_sample$condition)) {
  
  print(i)
  
  bc_df_sample_i <- bc_df_sample %>% filter(condition == i)
  bc_df_sample_i <- bc_df_sample_i[,colSums(is.na(bc_df_sample_i))<nrow(bc_df_sample_i)]
  
  if (nrow(bc_df_sample_i) > 300) {
  
  n <- sample(1:nrow(bc_df_sample_i), 300)

  
  p <- ggpairs(bc_df_sample_i %>% dplyr::select(-reporter_id, -condition),
               upper = list(continuous = corColor),
               lower = list(continuous = function(data, mapping, ...) {
                   ggally_points(data = data[n, ], mapping = mapping, alpha = 0.1, size = 0.5) +
                   geom_abline(slope = 1, lty = "dashed", col = "red") +
...
### Remove conditions based on low correlation scores
remove_conditions <- c("U2OS_r1_gcf6881", "U2OS_r2_gcf6881", "U2OS_r1_gcf6210", "mES_2i_LIF_r2_gcf7218", "mES_2i_LIF_r1_gcf7218")
...
## Calculate fold-changes of perturbations - keep only perturbations that change activities robustly
```{r}
