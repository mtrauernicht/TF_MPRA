...
metadata_df <- read_csv2("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/mt20231211_metadata_all.csv") %>%
...
bc_df_pert <- bc_df_cDNA_filt %>%
  ...
  distinct(mean_activity_sample_minP, reporter_id, condition, unique_sample, reference_condition_gcf, tf, target, off_target) %>% 
  mutate(mean_activity_sample_minP = log2(mean_activity_sample_minP)) %>%
...
bc_df_pert_ref <- bc_df_cDNA_filt %>%
  mutate(tf = gsub("_.*", "", tf)) %>%
  filter(is.na(stimulation) | condition == "mES_Dox", neg_ctrls == "No") %>%
...
bc_df_pert_dif <- bc_df_pert %>%
  left_join(bc_df_pert_ref) %>%
  mutate(activity_dif = mean_activity_sample_minP - ref_activity) %>%
  #mutate(activity_dif = ifelse((mean_activity_sample_minP < 0.5 | ref_activity < 0.5) & activity_dif < 1, 0, activity_dif)) %>%
  dplyr::select(-mean_activity_sample_minP, -ref_activity, -reference_condition_gcf) %>%
  spread(unique_sample, activity_dif)

## Keep conditions where replicates have the same effect in general on TF reporters
cor_all <- data.frame("condition" = unique(bc_df_pert_dif$condition),
                      "cor_all" = "")

for (i in unique(bc_df_pert_dif$condition)) {
  
  print(i)
  
  bc_df_pert_dif_i <- bc_df_pert_dif %>% filter(condition == i)
  bc_df_pert_dif_i <- bc_df_pert_dif_i[,colSums(is.na(bc_df_pert_dif_i))<nrow(bc_df_pert_dif_i)]
  
  if (ncol(bc_df_pert_dif_i) > 3) {
      
    cor_all$cor_all[cor_all$condition == i] <- cor.test(bc_df_pert_dif_i[,6], bc_df_pert_dif_i[,7], na.rm = T)$estimate
    
    p <- ggpairs(bc_df_pert_dif_i %>% dplyr::select(-),
               upper = list(continuous = corColor),
               lower = list(continuous = function(data, mapping, ...) {
...
bc_df_pert_dif_long <- bc_df_pert %>%
  ...
  mutate(tf_target2 = as.numeric(str_detect(target, paste0("\\b", tf, "\\b")))) %>%
  arrange(condition)
...
```{r}
### Remove conditions based on correlation scores - for some cell types I have more than enough data so I can afford to lose noisier replicates
remove_conditions <- c("U2OS_r1_gcf6881", "U2OS_r2_gcf6881", "U2OS_r1_gcf6210", "mES_2i_LIF_r2_gcf7218", "mES_2i_LIF_r1_gcf7218", "NPC_r1_gcf7218", "mES_2i_LIF_r3_gcf7218", "mES_2i_LIF_r2_gcf6578", "MCF7_r1_gcf6881", "HCT116_r1_gcf6881", "A549_r2_gcf7218")

bc_df_cDNA_filt <- bc_df_cDNA_filt %>%
  filter(!sample_id %in% remove_conditions)

### Remove perturbation conditions that don't correlate with other replicates
remove_conditions_pert <- c("mES_N2B27_r2_gcf5927", "mES_LIF_PD_r1_gcf6578", "K562_T3_r1_gcf7264", "mES_vitA_r2_gcf5927",
                            "A549_Nutlin_r2_gcf6210")


### Remove perturbations for which I don't have any evidence of effectiveness
remove_weak <- c("mES_POU5F1_r1", "mES_Nutlin_r2_gcf7447", "mES_Nutlin_r1_gcf7447", "mES_MEF2A_r1_gcf7027", "mES_GATA1-OE_r2_gcf7447", "mES_FOS::JUN_r1_gcf7027", "HepG2_HNF1A_r2_gcf7534", "HepG2_CEBPB_r1_gcf6952")

remove_conditions_pert <- c("mES_GSK4716_r1_gcf7218", "mES_GSK4716_r3_gcf7218")
...
plt <- ggpairs(bc_df_rep2 %>% filter(library == "1+2") %>% dplyr::select('Barcode 1', 'Barcode 2', 'Barcode 3', 'Barcode 4', 'Barcode 5', 'Barcode 6', 'Barcode 7', 'Barcode 8'),
               ...
               lower = list(continuous = function(data, mapping, ...) {
                 geom_abline(slope = 1, lty = "dashed", col = "red") +
                   ggally_points(data = data[n, ], mapping = mapping, stroke = NA, alpha = 0.1, size = 0.5) +
                   theme_pubr(border = T)}),
               diag = list(continuous = function(data, mapping, ...) {
                   ggally_densityDiag(data = data, mapping = mapping, alpha = 0.3, fill = "red") +
                   theme_pubr(border = T)})) +
...
bc_df_cDNA_filt$reporter_activity_minP_gcf <- ave(bc_df_cDNA_filt$mean_activity_sample_minP, bc_df_cDNA_filt$reporter_id, bc_df_cDNA_filt$condition,
                                                 bc_df_cDNA_filt$gcf, FUN = function(x) mean(x))
