...
bc_fraction <- bc_fraction %>%
  mutate(sample_id = paste(sample_id, library, sep = "_"))
...
pDNA <- merge(pMT02, pMT09, all = T, by = "barcode")
pDNA <- merge(pDNA, pMT09_02, all = T, by = "barcode")


bc_df_2 <- merge(pDNA, bc_df_filt, all = T, by = "barcode")
...
bc_df_cDNA_filt2 <- read_csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6578_gen2_stimulation-2/test2_try2.csv") %>%
  dplyr::select(reporter_id, condition, 'pval_neg2' = pval_neg) %>%
  unique()

bc_df_cDNA_filt3 <- merge(bc_df_cDNA_filt, bc_df_cDNA_filt2, by = c("reporter_id", "condition"))
plot(bc_df_cDNA_filt3$pval_neg, bc_df_cDNA_filt3$pval_neg2)
```
