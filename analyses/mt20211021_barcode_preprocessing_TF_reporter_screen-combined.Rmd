---
title: "Barcode preprocessing - TF reporter library 90 TFs - combined"
author: 
...
### Aim
In more than 100 samples, 35,500 reporters for 86 TFs were transfected into various cells (mainly mES, K562 & HepG2), and stimulated with various compounds. All the barcode counts from the 7 different sequencing runs are combined in this analysis. In this scripts, sanity checks will be carried out to filter out samples with bad quality, and barcode counts will be normalized to compute reporter activitites. 
To be more precise, the data consists of two different libraries, library 1, which is more focused on mES biology, and library 2, which is more focused on classical signaling pathways.
...
metadata_df <- read_csv2("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6578_gen2_stimulation-2/mt20211025_metadata_all_experiments.csv")
...
for (i in 1:length(bc_files)) {
  if (i == 1) {
  bc_df <- data.frame(bc_files[i])
  bc_df[3] <- names(bc_files[i])
  names(bc_df) <- c("barcode", "starcode_counts", "sample_id")
  }
  else {
  bc_df_i <- data.frame(bc_files[i])
  bc_df_i[3] <- names(bc_files[i])
  names(bc_df_i) <- c("barcode", "starcode_counts", "sample_id")
...
bc_df <- merge(bc_df, metadata_selected, all = T)
...
# Compare pDNA data
plot(bc_df_cor$pMT09_02_r1_gcf6502, bc_df_cor$pMT09_02_r1_gcf6578)
plot(bc_df_cor$pMT09_r1_gcf6578, bc_df_cor$pMT09_r1_gcf6502)


# Convert into data.frame with row.names for deseq2
bc_df_cor2 <- bc_df_cor %>%
  filter(library == "2") %>%
  dplyr::select(matches("barcode|gcf6578|pMT09_r2_gcf6502|pMT09_r1_gcf6502")) %>%
  dplyr::select(-pMT09_02_r1_gcf6578) %>%
  column_to_rownames("barcode") 

bc_df_cor3 <- bc_df_cor %>%
  filter(library == "1") %>%
  dplyr::select(matches("barcode|gcf5927|gcf6139|gcf6210|gcf6301")) %>%
  column_to_rownames("barcode") 

bc_df_cor4 <- bc_df_cor %>%
  filter(library == "1+2") %>%
  dplyr::select(matches("barcode|gcf6502|pMT09_02_r1_gcf6578")) %>%
  dplyr::select(-pMT09_r1_gcf6502, -pMT09_r2_gcf6502) %>%
  column_to_rownames("barcode") 
...
cor_df <- cor(bc_df_cor3, method = "pearson")
...
ggplot_cust(bc_df_2, aes(x = rpm)) +
  geom_histogram(binwidth = 20) +
  xlim(0,250)+
  ylim(0,6000)+
  facet_wrap(~sample_id)

ggplot_cust(bc_df[bc_df$rpm >= 500 & !is.na(bc_df$tf),], aes(x = rpm)) +
  geom_histogram(binwidth = 20) +
  xlim(250,1000)+
  ylim(0,50)+
  facet_wrap(~sample_id)
...
for (i in unique(bc_fraction$cutoff)) {
  for (j in unique(bc_fraction$sample_id)) {
    for (k in unique(bc_fraction$library)) {
    bc_fraction$bcs_found[bc_fraction$cutoff == i & bc_fraction$sample == j & bc_fraction$library == k] <- 
      nrow(bc_df_filt[bc_df_filt$rpm >= i & bc_df_filt$sample_id == j & bc_df_filt$library == k,])/
      bc_counts_design$bc_counts[bc_counts_design$library == k] *100
    }
  }
}
...
for (i in unique(bc_reads$cutoff)) {
  for (j in unique(bc_df$sample_id)) {
    for (k in unique(bc_reads$library)) {
      bc_reads$bc_reads[bc_reads$cutoff == i & bc_reads$sample_id == j & bc_reads$library == k] <- 
        sum(na.omit(bc_df_filt$rpm[bc_df_filt$rpm >= i & bc_df_filt$sample_id == j & bc_df_filt$library == k]))/
      sum(bc_df$rpm[bc_df$rpm >= i & bc_df$sample_id == j & (bc_df$library == k | bc_df$match == "No")]) *100
    }
  }
}
...
bc_fraction <- merge(bc_fraction, bc_reads, by = c("sample_id", "library", "cutoff"))
...
for (i in unique(bc_fraction$library)) {
p <- ggplot_cust(bc_fraction[bc_fraction$library == i,]) +
...
pMT02 <- bc_df_filt %>%
  filter(str_detect(sample_id, "pMT02"), library == "1") %>%
  dplyr::select(barcode, sample_id, rpm) %>%
  spread(sample_id, rpm) %>%
  mutate(pMT02 = (pMT02_r1_gcf5927 + pMT02_r1_gcf6301 + pMT02_r2_gcf5927)/3) %>%
  dplyr::select(barcode, pMT02)

pMT09 <- bc_df_filt %>%
  filter(str_detect(sample_id, "pMT09"), library == "2") %>%
  dplyr::select(barcode, sample_id, rpm) %>%
  spread(sample_id, rpm) %>%
  mutate(pMT09 = (pMT09_r1_gcf6502 + pMT09_r1_gcf6578 + pMT09_r2_gcf6502 + pMT09_r3_gcf6578)/4) %>%
  dplyr::select(barcode, pMT09)

pMT09_02 <- bc_df_filt %>%
  filter(str_detect(sample_id, "pMT09_02"), library == "1+2") %>%
  dplyr::select(barcode, sample_id, rpm) %>%
  spread(sample_id, rpm) %>%
  mutate(pMT09_02 = (pMT09_02_r1_gcf6502 + pMT09_02_r1_gcf6578)/2) %>%
  dplyr::select(barcode, pMT09_02)

pDNA <- merge(pMT02, pMT09, all = T)
pDNA <- merge(pDNA, pMT09_02, all = T)


bc_df_2 <- merge(pDNA, bc_df_filt, all = T)
...
bc_df_2$sample_id <- paste(bc_df_2$sample_id, bc_df_2$library, sep = "_")
cor <- data.frame("sample_id" = unique(bc_df_2$sample_id), "cor" = "", stringsAsFactors = F) %>%
  mutate(library = gsub(".*_(.*$)", "\\1", sample_id))

for (i in unique(bc_df_2$sample_id)) {
  for (j in unique(bc_df_2$library)) {
    if (j == "1") {
      cor$cor[cor$sample_id == i & cor$library == j] <- stats::cor(bc_df_2$rpm[bc_df_2$sample_id == i], bc_df_2$pMT02[bc_df_2$sample_id == i], use = "pairwise.complete.obs")
    }
    if (j == "1+2") {
      cor$cor[cor$sample_id == i & cor$library == j] <- stats::cor(bc_df_2$rpm[bc_df_2$sample_id == i], bc_df_2$pMT09_02[bc_df_2$sample_id == i], use = "pairwise.complete.obs")
    }
        if (j == "2") {
      cor$cor[cor$sample_id == i & cor$library == j] <- stats::cor(bc_df_2$rpm[bc_df_2$sample_id == i], bc_df_2$pMT09[bc_df_2$sample_id == i], use = "pairwise.complete.obs")
    }
  }
}
...
bc_df <- merge(bc_df, cor_2, by = c("sample_id", "library"), all = T)
...
bc_df_rep <- bc_df[!is.na(bc_df$tf),] %>% 
  dplyr::select(replicate, rpm, barcode, gcf, condition, library, neg_ctrls) %>% 
  unique() %>%
  mutate(replicate = paste(library, replicate, sep = "_")) %>%
  spread(replicate, rpm)

colors <- c("#2D3047", "#1B998B")

for (i in unique(bc_df_rep$library)) {
p <- ggscatter(bc_df_rep[bc_df_rep$library == i,], x = paste(i, "r1", sep = "_"), y = paste(i, "r2", sep = "_"),
   add = "reg.line",
   color = "neg_ctrls",
   size = 1,
...
for (i in unique(bc_df_rep$library)) {
p <- ggscatter(bc_df_rep[bc_df_rep$library == i,], x = paste(i, "r1", sep = "_"), y = paste(i, "r3", sep = "_"),
   add = "reg.line",
   color = "neg_ctrls",
   size = 1,
...
for (i in unique(bc_df_rep$library)) {
p <- ggscatter(bc_df_rep[bc_df_rep$library == i,], x = paste(i, "r2", sep = "_"), y = paste(i, "r3", sep = "_"),
   add = "reg.line",
   color = "neg_ctrls",
   size = 1,
...
# Manually remove weird K562 sample
bc_df <- bc_df %>%
  filter(sample_id != "K562_DMSO_r3_gcf6502")
...
pMT02 <- bc_df %>%
  filter(str_detect(sample_id, "pMT02"), library == "1") %>%
  filter(starcode_counts > 20) %>%
  dplyr::select(barcode, sample_id, rpm) %>%
  spread(sample_id, rpm) %>%
  mutate(pMT02 = dplyr::select(., -matches("barcode")) %>% rowMeans(., na.rm = T)) %>%
  dplyr::select(barcode, pMT02)
...
pMT09 <- bc_df %>%
  ...
  dplyr::select(barcode, sample_id, rpm) %>%
  spread(sample_id, rpm) %>%
  mutate(pMT09 = dplyr::select(., -matches("barcode")) %>% rowMeans(., na.rm = T)) %>%
  dplyr::select(barcode, pMT09)
...
pMT09_02 <- bc_df %>%
  ...
  dplyr::select(barcode, sample_id, rpm) %>%
  spread(sample_id, rpm) %>%
  mutate(pMT09_02 = dplyr::select(., -matches("barcode")) %>% rowMeans(., na.rm = T)) %>%
  dplyr::select(barcode, pMT09_02)
...
bc_df <- merge(bc_df, pDNA, by = "barcode")

# Compute activity by dividing cDNA bc counts through pDNA bc counts
bc_df$activity[bc_df$library == "1"] <- bc_df$rpm[bc_df$library == "1"] / bc_df$pMT02[bc_df$library == "1"]
bc_df$activity[bc_df$library == "1+2"] <- bc_df$rpm[bc_df$library == "1+2"] / bc_df$pMT09_02[bc_df$library == "1+2"]
bc_df$activity[bc_df$library == "2"] <- bc_df$rpm[bc_df$library == "2"] / bc_df$pMT09[bc_df$library == "2"]
...
ggplot_cust(outlier, aes(x = barcode_number, y = activity)) +
  geom_quasirandom() +
  xlab("barcode #") + 
...
bc_df_cDNA_filt_neg <- bc_df_cDNA_filt %>%
  ...
  filter(str_detect(tf, "RANDOM"), promoter == "minP") %>%
  unique() %>%
  mutate(activity = ave(activity, sample_id, FUN = function(x) mean(x))) %>%
  dplyr::select(-reporter_id) %>%
  unique() %>%
  dplyr::select("minP_activity" = activity, sample_id) %>%
  unique()

bc_df_cDNA_filt <- merge(bc_df_cDNA_filt, bc_df_cDNA_filt_neg, by = "sample_id")
bc_df_cDNA_filt <- bc_df_cDNA_filt %>%
  mutate(minP_activity = activity / minP_activity)

## Compute activity relative to SP1 reporter
...
bc_df_cDNA_filt <- bc_df_cDNA_filt %>% 
  dplyr::select(-upper_activity, -lower_activity, 
         -high_outlier, -low_outlier, -match, -pMT02, -pMT09, -pMT09_02, -mean_activity, -sd_activity)
