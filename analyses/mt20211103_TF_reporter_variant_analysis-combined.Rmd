...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6881_gen2_all/results/mt20220527_reporter_activity_filt_combined.csv", header = T) %>%
...
cDNA_df2 <- cDNA_df %>%
  dplyr::filter(library == "1+2" & condition == "K562_DMSO" & tf == "TP53")
...
# Investigate pMT02 data in HepG2
ggplot(cDNA_df %>%
  dplyr::filter(nchar == 12, condition == "HepG2_CDCA") %>%
  distinct(replicate, starcode_counts, reporter_id, barcode, tf) %>%
  spread(key = "replicate", value = "starcode_counts"),
       aes(x = r1, y = r2)) +
  geom_point()
...
# cDNA_df4 <- merge(cDNA_df3, cDNA_df2)



cDNA_df <- merge(cDNA_df, cDNA_df2, by = c("tf", "condition"), all = T) %>%
  ...
  mutate(pval_neg = ifelse(neg_active == "yes" | is.na(pval_neg), pval_minP, pval_neg)) 
...
cDNA_df$reporter_rpm <- ave(cDNA_df$rpm, cDNA_df$reporter_id, FUN = mean)
cDNA_df$pval_adj_minP <- adj_pvalues(ihw(pval_minP ~ reporter_rpm,  data = cDNA_df, alpha = 0.1))
cDNA_df$pval_adj_neg <- adj_pvalues(ihw(pval_neg ~ reporter_rpm,  data = cDNA_df, alpha = 0.1))

cDNA_df4 <- cDNA_df %>%
  dplyr::filter(neg_ctrls == "No", commercial_reporter == "No", hPGK == "No", str_detect(tf, "RANDOM", negate = T), native_enhancer == "No") %>%
  ...
  mutate(n_tfs = as.numeric(ave(tf, condition, FUN = function(x) unique(length(x))))) %>%
  dplyr::filter(n_tfs == 86) %>%
  dplyr::select(-n_tfs)

cDNA_df4$reporter_activity_minP[cDNA_df4$pval_adj_minP < -log10(0.15) | cDNA_df4$reporter_activity_minP < 1.5 & cDNA_df4$reporter_activity_minP > -0.5] <- 0 # Select only reporters that are significantly active
...
cDNA_df4$cell_type[cDNA_df4$cell_type == "mES_serum"] <- "mES"
cDNA_df4$cell_type[cDNA_df4$cell_type == "mES_Sox2"] <- "mES"
cDNA_df4$cell_type[cDNA_df4$cell_type %in% c("K562", "U2OS", "MCF7", "HEK293", "HCT116", "A549")] <- "cancer"
pca_res <- prcomp(cDNA_df4 %>% dplyr::select(-cell_type), scale. = TRUE)
loadings <- data.frame('scale' = pca_res$scale) %>%
  dplyr::filter(scale > 0.5)
g <- ggbiplot(pca_res, group = cDNA_df4$cell_type, ellipse = T)
...
cDNA_df2 <- cDNA_df %>%
                  dplyr::filter(condition == "mES_2i_LIF", neg_ctrls == "No") %>% ## Change condition here to look at other conditions
...
ggplot(mapping = aes(x = reporter_activity_minP, y = pval_adj_minP)) +
    ...
    geom_point(data = cDNA_df2 %>% dplyr::filter(abs(reporter_activity_minP) <= 1 | pval_adj_minP <= -log10(0.05) 
                                          #& pluri == "No"
                                          ), 
               size = 2, color = "grey90") +
    geom_point(data = cDNA_df2 %>% dplyr::filter(abs(reporter_activity_minP) > 1 & pval_adj_minP > -log10(0.05) 
...
cDNA_df_TF <- cDNA_df %>%
  ...
  mutate(pval_minP_adj = p.adjust(pval_minP, method = "fdr")) %>%
  mutate(tf_condition = paste(tf, condition, sep = "_")) %>%
  distinct(reporter_activity_minP, tf, condition, tf_condition, reporter_id, promoter, spacing, distance, background) %>%
  filter(tf_condition %in% lm_pval_df_sign$tf_condition) %>%
...
for (i in unique(cDNA_df_TF$tf_condition)) {
  print(i)
  x <- lm(log2(reporter_activity_minP) ~ 
                       promoter + 
                       spacing + 
                       distance + 
                       background,
