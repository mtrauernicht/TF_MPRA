...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf6578_gen2_stimulation-2/results/mt20211117_reporter_activity_filt_combined.csv", header = T)
```
...
ggplot_custom(cDNA_df %>%
                filter(condition == "HepG2_DMSO", neg_ctrls == "No", library == "1+2") %>%
                dplyr::select(reporter_id, pval_adj, reporter_activity_minP, promoter) %>%
                unique(),
            aes(x = log(reporter_activity_minP), y = -log10(pval_adj), 
                color = ifelse(log(reporter_activity_minP) > 1 & pval_adj < 0.05, "green", 
...
for (i in unique(cDNA_df2$condition)) {
  p <- ggplot_custom(cDNA_df2 %>%
                       filter(condition == i),
                     aes(x = reorder(tf, -class_count), fill = factor(reporter_activity_class, levels = c("0", "1", "-1")))) +
...
cDNA_df3$class[cDNA_df3$`0` >= 0.66] <- 0
cDNA_df3$class[cDNA_df3$`1` >= 0.66] <- 2
cDNA_df3$class[cDNA_df3$`-1` >= 0.66] <- -2
cDNA_df3$class[cDNA_df3$`1` >= 0.33 & cDNA_df3$`1` < 0.66] <- 1
cDNA_df3$class[cDNA_df3$`-1` >= 0.33 & cDNA_df3$`-1` < 0.66] <- -2
...
cols <- makeColorRampPalette(c(colors_diverse[3], colors_diverse[1], colors_diverse[2]), # distances 3 to max(distmat) colored from green to black
                             0.5,
                             100)
...
mixed_classes <- cDNA_df3 %>%
  ...
  filter(class == 1 | class == -2 | class == -1) %>%
  unique()
...
pheatmap(as.matrix(t(exp_features_sel_heatmap)),
