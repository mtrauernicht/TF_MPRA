...
bc_df <- bc_df[bc_df$n_bc >= 150 | bc_df$sample == "pMT02_pDNA" | bc_df$sample == "pMT09_r1_r2" | 
                 bc_df$sample == "pMT09_r3" ,] %>% dplyr::select(-n_bc)

# Remove samples with high pDNA contamination
bc_df <- bc_df[bc_df$cor <= 0.85 | bc_df$sample == "pMT02_pDNA" | bc_df$sample == "pMT09_r1_r2" | 
                 bc_df$sample == "pMT09_r3",] %>% dplyr::select(-cor)
...
pDNA_lib2 <- bc_df[bc_df$sample == "pMT09_r1_r2" | bc_df$sample == "pMT09_r3",] %>%
  filter(library == "2") %>%
...
bc_df$noisy <- "No"
bc_df$noisy[bc_df$pDNA_counts_rpm <= 7] <- "Yes"
```
...
bc_df_cDNA_filt_neg <- bc_df_cDNA_filt %>%
  dplyr::select(sample, activity, reporter_id, neg_ctrls, rand_promoter, library, noisy) %>%
  filter(neg_ctrls == "Yes", rand_promoter == "Yes", noisy == "No") %>%
  unique() %>%
  mutate(activity = ave(activity, library, FUN = function(x) mean(x))) %>%
