---
title: "Barcode preprocessing - TF reporter library 90 TFs - KD screen mES"
author: 
...
### Aim
35,500 reporters for 86 TFs were transfected into mES cells that were previously treated with ~35 different siRNAs for TFs. In this script the barcode counts from these samples will be pre-processed.
...
metadata_df <- read_csv2("/DATA/usr/m.trauernicht/projects/SuRE-TF/data/gcf7027_KD_mES/mt20220930_metadata_gcf7027_KD.csv")
...
bc_df_cor <- bc_df_cor %>%
  filter(sample_id %in% c("pMT02_3_r1_gcf6952", "pMT02_r1_gcf6952", "pMT02_v2_r1_gcf6952", "pMT02_v3_r1_gcf6952", "pMT02-09_v2_r1_gcf6952", "pMT02-09_v3_r1_gcf6952", "E1961_pDNA_r1_gcf6992")) %>%
  dplyr::select(sample_id, barcode, rpm) %>%
  unique() %>%
...
# Compare pDNA data
plot(bc_df_cor$E1961_pDNA_r1_gcf6992, bc_df_cor$`pMT02-09_v2_r1_gcf6952`)
# 
# 
...
pMT09_02 <- bc_df_filt %>%
  filter(str_detect(sample_id, "pDNA_r1_gcf7027"), library == "1+2") %>%
  dplyr::select(barcode, sample_id, rpm) %>%
  spread(sample_id, rpm) %>%
  #mutate(pMT09_02 = (`pMT02-09_v2_r1_gcf6952` + `pMT02-09_v3_r1_gcf6952`)/2) %>%
  dplyr::select(barcode, "pMT09_02" = `pDNA_r1_gcf7027`)
...
bc_df <- bc_df %>%
  filter(n_bc > 75 | str_detect(sample_id, "pMT") | str_detect(sample_id, "pDNA")) %>% 
  dplyr::select(-n_bc)
...
bc_df <- bc_df %>%
  filter(cor <= 0.5 | str_detect(sample_id, "pMT")| str_detect(sample_id, "pDNA")) %>% 
           dplyr::select(-cor)
...
pMT09_02 <- bc_df %>%
  filter(sample_id == "pDNA_r1_gcf7027") %>%
   ...
   dplyr::select(barcode, "E2035_pDNA" = rpm)
...
bc_df$activity <- bc_df$rpm / bc_df$E2035_pDNA
...
bc_df_cDNA$deviation <- bc_df_cDNA$activity / bc_df_cDNA$mean_activity
...
bc_df_cDNA_filt <- bc_df_cDNA_filt %>%
  dplyr::select(-match, -E2035_pDNA, -mean_activity, -deviation, -n_reporters)
