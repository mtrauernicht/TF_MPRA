...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/tf_activity_reporter/data/SuRE_TF_1/results/mt20200512_reporter_activity_filt.csv", header = T)
...
ggplot(cDNA_df[cDNA_df$native_enhancer == "No" &
  ...
  labs(title = "bc-count distribution cDNA vs. pDNA") +
  scale_fill_manual(values = colors) +
...
sp <- ggscatter(cDNA_df[cDNA_df$native_enhancer == "No" &
                    ...
                    cDNA_df$hPGK == "No" &
                    cDNA_df$rand_promoter == "No",],
          size = 0.1, color = "#2D3047", alpha = 0.1,
          y = "activity", x = "pDNA_counts_rpm",
          title = "reporter pDNA counts vs reporter activity",
          ylab = "reporter activity",
          xlab = "pDNA counts per million",
          facet.by = "TF")
sp + xlim(0,200) + ylim(0,20)
...
sp <- ggscatter(cDNA_df[cDNA_df$TF == "Gbx2" &
          ...
          y = "activity", x = "pDNA_counts_rpm",
          title = "Gbx2: reporter pDNA counts vs reporter activity",
          ylab = "reporter activity",
          xlab = "pDNA counts per million",
          facet.by = "condition")
sp + xlim(0,200) + ylim(0,20)
## Conclusion: This really seems to be noise - Do I need to take care of this?
...
## Log-linear expression modelling to explain variance - all data
```{r log-linear-model, out.width= "100%", fig.align= "center", echo=FALSE, warning= FALSE}
# Fit model using lm to predict expression from the independent contribution of design parameters
## Parameters: Spacing, distance, background, promoter, WT vs. mutated

### Start with all TFs at the same time
cDNA_df_TF <- cDNA_df[cDNA_df$native_enhancer == "No" &
                        cDNA_df$hPGK == "No" & 
                        cDNA_df$rand_promoter == "No",] %>% 
  mutate(log_reporter_activity = ave(log_reporter_activity, reporter, condition, FUN = function(x) mean(x))) %>%
  select(log_reporter_activity, TF, condition, promoter, spacing, distance, background) %>%
  unique()

### Calculate expression variance
exp_variance <- lm(log_reporter_activity ~ TF + condition + promoter + spacing + distance + background , cDNA_df_TF)
cDNA_df_TF$activity_predicted <- exp_variance$fitted.values

exp_variance <- anova(exp_variance) %>% rownames_to_column("feature") %>%
...
ggplot(exp_variance, 
       aes(x = reorder(feature, -rel_sum_sq), y = rel_sum_sq)) +
...
sp + stat_cor(method = "pearson", label.x = 1.5, label.y = -2.4) + geom_abline(linetype = "dashed")
```
...
```{r log-linear-model-2, out.width= "100%", fig.align= "center", echo=FALSE, warning= FALSE}
# For each TF separately
cDNA_df_TF <- cDNA_df[cDNA_df$native_enhancer == "No" &
                        cDNA_df$hPGK == "No" & 
                        cDNA_df$rand_promoter == "No",] %>% 
  mutate(log_reporter_activity = ave(log_reporter_activity, reporter, condition, FUN = function(x) mean(x))) %>%
  select(log_reporter_activity, TF, neg_ctrls, condition, promoter, spacing, distance, background) %>%
  unique() %>%
  mutate(TF = gsub("_neg$", "", TF)) %>%
  setnames("neg_ctrls", "mutated")
cDNA_df_TF <- cDNA_df_TF[-grep("Random", cDNA_df_TF$TF),]


# Calculate feature importance of expression variance, predicted activity, and weights of the model
## Prepare dfs
exp_variance <- data.frame(c("mutated", "condition", "promoter", 
                             "spacing", "distance", "background", "Residuals"))
prediction <- data.frame(1:504)
weight <- data.frame(c("(Intercept)", "mutatedYes", "condition2i_pos_LIF", 
...
## Calculate log-linear model parameters using lm 
for (i in unique(cDNA_df_TF$TF)) {
  x <- lm(log_reporter_activity ~ mutated + condition + 
...
## Plot expression variance feature importance
ggplot(exp_variance, 
...
## Plot correlation of predicted activity and measured activity
sp <- ggscatter(cDNA_df_TF, x = "log_reporter_activity", y = "activity_predicted",
   add = "reg.line", 
   add.params = list(color = "blue", fill = "lightgray"),
   conf.int = TRUE, ylab = "Predicted log2-activity", xlab = "Average log2-activity",
...
ggplot(weight, 
       aes(x = reorder(feature, -value), y = value, fill = condition)) +
...
# Feature importance overview for each TF in a single plot (bar/pie)
```
