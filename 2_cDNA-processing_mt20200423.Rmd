...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/tf_activity_reporter/data/SuRE_TF_1/results/mt20200511_reporter_activity_filt.csv", header = T)
...
## reporter activity for each TF - comparing to original pDNA counts
...
ggplot(cDNA_df[cDNA_df$native_enhancer == "No" &
  ...
  geom_density(aes(x = reporter_activity, fill = "#1B998B", alpha = 0.4)) + 
  geom_density(aes(x = pDNA_counts, fill = "#2D3047", alpha = 0.4)) + 
  xlab("normalized reporter activity: cDNA/pDNA") + 
  labs(title = "reporter activity distribution - backgrounds") +
  scale_fill_manual(values = colors) +
  xlim(0,5) + theme_classic() +
  facet_wrap(~TF)
...
cDNA_df_TF <- cDNA_df[cDNA_df$native_enhancer == "No" &
                        cDNA_df$hPGK == "No" & 
                        cDNA_df$rand_promoter == "No",] %>% 
  mutate(log_reporter_activity = ave(log_reporter_activity, reporter, FUN = function(x) mean(x))) %>%
...
exp_variance$group <- "no residual"
exp_variance$group[grep("Residual", exp_variance$feature)] <- "residual"
z <- z %>% select(-id) %>% melt(variable.name = "TF") %>% na.omit() %>%
  select(-TF) %>% setnames("value", "activity_predicted")
cDNA_df_TF <- merge(cDNA_df_TF, z)
