...
cDNA_df <- read.csv("/DATA/usr/m.trauernicht/projects/tf_activity_reporter/data/SuRE_TF_1/results/mt20200527_reporter_activity_filt.csv", header = T)
...
ES <- c("Esrrb", "Gbx2", "Klf4", "Smad4", "Stat3", "Tfcp2l1", "Zfp42", "Zfx")
cDNA_df$group[grep(paste(ES, collapse = "|"), cDNA_df$TF)] <- "Stemness"
pos <- c("Sp1", "Nfya")
cDNA_df$group[grep(paste(pos, collapse = "|"), cDNA_df$TF)] <- "Positive Control"
cDNA_df$group[grep("Random", cDNA_df$TF)] <- "Negative Control"
...
annotation_row <- data.frame("TF" = tf_activity_heatmap$TF, 
                             "group" = tf_activity_heatmap$group) %>%
  remove_rownames %>% column_to_rownames(var="TF")

annotation_colors = list(
    group = c(Other ="#1B998B", `Neural induction` = "#2D3047", Stemness = "#FF9B71", `Positive Control` = "#FFFFC7", `Negative Control` = "#B0413E"))
...
for (i in unique(cDNA_df_TF$TF)) {
  ...
  sp <- ggscatter(cDNA_df_TF[cDNA_df_TF$TF == i,], x = "log_reporter_activity", y = "activity_predicted",
   ...
   title = i,
   xlab = "Average log2-activity") + theme_bw() +
  stat_cor(method = "pearson", label.x = -3, label.y = 3.7) + geom_abline(linetype = "dashed")
