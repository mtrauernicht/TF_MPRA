...
annotation_colors = list(
    group = c(Other ="#1B998B", `Neural induction` = "#2D3047", Stemness = "#FF9B71"))
...
for (i in unique(cDNA_df_TF$TF)) {
  ...
  facet_wrap_paginate(~TF, nrow = 1, ncol = 1, page = i)
  
  
  # Feature importance overview for each TF (including mean activities per feature)
  fp <- ggplot(x, aes(fill=condition, y=activity, x=reorder(group, activity))) +
  geom_bar(position="stack", stat="identity") +
  coord_flip() +
  scale_fill_manual(values = colors) +
  #scale_fill_viridis(discrete = T) +
  ylab("Variance explained (%)") + xlab("") + 
  labs(title = "Linear activity variance modelling", 
       subtitle = "Feature colors scaled using mean activity data") +
  theme_bw() +
  theme(text = element_text(size = 12)) +
  ...
  labs(title = "Linear variance modelling", 
       subtitle = paste("Weights used in the linear model, page", i)) +
  theme_bw() +
  theme(text = element_text(size = 12)) +
  ...
  # Combine all plots
  sp.plot <- arrangeGrob(sp,p,fp, nrow = 2, ncol = 2)
