...
```{r}
# Figure S4E: Plot perturbation response of all TFs
## Prepare data frame for following analyses
cDNA_df2 <- cDNA_df %>%
  mutate(tf = gsub("_.*", "", tf)) %>%
  dplyr::filter(neg_ctrls == "No", 
...
## Figure S4F: Plot perturbation response of NR1H4 reporters to all perturbations
ggplot(cDNA_df %>%
         mutate(tf = gsub("_.*", "", tf)) %>%
...
### Figure S6: which thresholds do the primer reporters fullfill?
...
## Plot original and mutated reporter activity
