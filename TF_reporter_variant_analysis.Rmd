...
cDNA_df <- read.csv("/Users/m.trauernicht/root/usr/m.trauernicht/projects/SuRE-TF/data/gcf7124_stimulations/results/mt20240124_reporter_activity_filt_combined.csv", header = T)
...
# Load RNA-seq data
tf_rna <- read_tsv("/Users/m.trauernicht/root/usr/m.trauernicht/data/RNA_seq/rna_tpm_all_tfs.tsv")
...
## Correlation with other TFs
```{r}
cDNA_df3 <- cDNA_df2 %>%
  dplyr::select(-tf) %>%
  merge(tf_rna %>% filter(tf %in% unique(cDNA_df2$tf))) %>%
  mutate(TPM = ifelse(is.na(TPM), 0, TPM)) %>%
  mutate(nTPM = ifelse(is.na(nTPM), 0, nTPM)) %>%
  mutate(log_nTPM = log2(nTPM + 0.01)) %>%
  mutate(range_TPM = quantile(log_nTPM, probs = 0.95) - quantile(log_nTPM, probs = 0.05)) %>%
...
for (i in unique(cDNA_df3$reporter_id)) {
  ...
  for (j in unique(cDNA_df3$tf)) {
  
    if (nrow(cDNA_df3[cDNA_df3$reporter_id == i,]) > 4) {
      
      print(j)
      
      cDNA_df3$cor_pval[cDNA_df3$reporter_id == i & cDNA_df3$tf == j] <- cor.test(2^cDNA_df3$reporter_activity_minP[cDNA_df3$reporter_id == i & cDNA_df3$tf == j],
                                                                    cDNA_df3$nTPM[cDNA_df3$reporter_id == i & cDNA_df3$tf == j], method = "pearson", exact = F)$p.value
    
      cDNA_df3$cor[cDNA_df3$reporter_id == i & cDNA_df3$tf == j] <- cor(2^cDNA_df3$reporter_activity_minP[cDNA_df3$reporter_id == i & cDNA_df3$tf == j], 
                                                 cDNA_df3$nTPM[cDNA_df3$reporter_id == i & cDNA_df3$tf == j], method = "pearson")
      }
...
cDNA_df_cor <- cDNA_df3 %>%
  distinct(reporter_id, tf, cor) %>%
  spread(key = "tf", value = "cor") %>%
  mutate(tf = gsub("_.*", "", reporter_id))

for (i in unique(cDNA_df_cor$tf)) {
  
  cDNA_df_cor_i <- cDNA_df_cor %>%
    filter(tf == i) %>%
    dplyr::select(-tf) %>%
    column_to_rownames("reporter_id")
  
  p <- pheatmap(cDNA_df_cor_i, 
                show_rownames = F,
                main = i, cellwidth = 8, 
                cellheight = 8,  
...
```{r, warning=F}
# proteomics_df_long <- read_tsv("/Users/m.trauernicht/root/usr/m.trauernicht/data/RNA_seq/protein_abundance_all_tfs.tsv")
# 
# ## Combine TF reporter data with RNA-seq data
...
tf_properties <- read_csv2("/Users/m.trauernicht/root/usr/m.trauernicht/projects/SuRE-TF/data/tf_binding_characterization.csv")
...
ggraph(routes_igraph, layout = "stress") +
  ...
  theme_graph()
```
Looks interesting, but I wouldn't know what to focus on. 
...
```{r}
# Load RNA-seq data
KD_tf_changes <- read_csv("/Users/m.trauernicht/root/usr/m.trauernicht/projects/SuRE-TF/data/gene_counts_tpm_KD.csv") %>%
...
### Explore activities of optimal reporters upon KDs in mES
```{r}
### Activities upon KD
optimal_reporters <- cDNA_df_confidence2 %>%
  group_by(tf) %>%
...
sox2_degr <- readRDS("/Users/m.trauernicht/root/usr/m.trauernicht/projects/SuRE-TF/data/DE_SOX2_degron.rds")
